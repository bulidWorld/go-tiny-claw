...
type Message struct {
	...
	// ReasoningContent 存放模型的内部推理过程 (如 DeepSeek thinking mode 的 reasoning_content)
	...
	// ToolResults 支持在一条 user 消息中携带多个工具执行结果。
	// Anthropic/DeepSeek API 要求：同一 assistant 消息中所有 tool_use 的 tool_result
	// 必须放在紧接着的同一条 user 消息里，不能拆成多条。
